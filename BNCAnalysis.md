...
<div class="chunk" id="loadmesc"><div class="rcode"><div class="source"><pre class="knitr r">## Supplementary table 1, sheet 1
...
<div class="chunk" id="unnamed-chunk-1"><div class="rcode"><div class="source"><pre class="knitr r">eg <- as.data.table(as.data.frame(org.Mm.egSYMBOL2EG))
setkey(eg, gene_id)
...
<div class="chunk" id="unnamed-chunk-2"><div class="rcode"><div class="source"><pre class="knitr r">mescFdat <- fdat[colnames(mesc),,mult='first']
...
This affects <code class="knitr inline">77</code> records.

## Loading mouse T-cell data
We repeat the process as above.
<div class="chunk" id="loadt"><div class="rcode"><div class="source"><pre class="knitr r">## Supplementary Data 1, sheet 1
...
<div class="chunk" id="unnamed-chunk-3"><div class="rcode"><div class="source"><pre class="knitr r">T_cell_matrix <- as.matrix(T_cell_raw[,-1,with=FALSE])
...
<div class="chunk" id="unnamed-chunk-4"><div class="rcode"><div class="source"><pre class="knitr r">TcellFdat <- fdat[colnames(T_cell_matrix),,mult='first']
...
There are <code class="knitr inline">182</code> cells and <code class="knitr inline">9571</code> genes in the mESC data and
<code class="knitr inline">81</code> cells and <code class="knitr inline">7071</code> genes in the T-cell data.
...
<div class="chunk" id="unnamed-chunk-5"><div class="rcode"><div class="source"><pre class="knitr r">lrTestLM <- function(Fit, drop){
...
<div class="chunk" id="ccR2"><div class="rcode"><div class="source"><pre class="knitr r">afit <- lm(mesc~cycle_class, data=mescCdat)
...
<div class="source"><pre class="knitr r">devSum <- devModels[,{ 
...
<div class="chunk" id="varianceEstimatesFigureS4"><div class="rcode"><div class="source"><pre class="knitr r">## right endpoints of R^2 bins
...
<div class="rimage default"><img src="figure/varianceEstimatesFigureS4-1.png" title="plot of chunk varianceEstimatesFigureS4" alt="plot of chunk varianceEstimatesFigureS4" class="plot" /></div>
</div></div>
#### Remark
...
<div class="chunk" id="unnamed-chunk-6"><div class="rcode"><div class="source"><pre class="knitr r">getPCA <- function(exprMat, cData, scale.=FALSE){
...
<div class="chunk" id="plotmescpca"><div class="rcode"><div class="source"><pre class="knitr r">ggpairs(mescPca$pcPairs, color='cycle_class')
</pre></div>
<div class="rimage default"><img src="figure/plotmescpca-1.png" title="plot of chunk plotmescpca" alt="plot of chunk plotmescpca" class="plot" /></div>
</div></div>
...
<div class="chunk" id="plotTcpca"><div class="rcode"><div class="source"><pre class="knitr r">tcpca <- getPCA(T_cell_matrix, TcellCdat[,c('panama', 'clusterid')])
ggpairs(tcpca$pcPairs, columns=c(1:5), color='clusterid')
</pre></div>
<div class="rimage default"><img src="figure/plotTcpca-1.png" title="plot of chunk plotTcpca" alt="plot of chunk plotTcpca" class="plot" /></div>
</div></div>


## Percentage variance due to PC1
<div class="source"><pre class="knitr r">varExplained <- data.frame('data set'=c('mESC', 'T-cell'),
...
<div class="chunk" id="PCAforClusters"><div class="rcode"><div class="source"><pre class="knitr r">tcCorrectedPCA <- getPCA(T_cell_matrix_corrected, TcellCdat[,c('panama', 'clusterid')])
...
<div class="rimage default"><img src="figure/PCAforClusters-1.png" title="plot of chunk PCAforClusters" alt="plot of chunk PCAforClusters" class="plot" /></div>
</div></div>
...
<div class="chunk" id="TSNEforClusters"><div class="rcode"><div class="source"><pre class="knitr r">tsne <- data.frame(tsne=Rtsne(T_cell_matrix, dims = 2, initial_dims = 50, perplexity = 20, theta = 0.5, check_duplicates = TRUE, pca = TRUE)$Y, geomsize=rowSums(T_cell_matrix))
...
<div class="rimage default"><img src="figure/TSNEforClusters-1.png" title="plot of chunk TSNEforClusters" alt="plot of chunk TSNEforClusters" class="plot" /></div>
</div></div>
...
<div class="source"><pre class="knitr r">models <- c('geomsizeUncorr ~ cycle_class', 'panama ~ cycle_class', 'panama ~ geomsizeUncorr +cycle_class')
...
<div class="chunk" id="gsea"><div class="rcode"><div class="source"><pre class="knitr r">eSet <- ExpressionSet(t(T_cell_matrix_corrected))
...
<div class="chunk" id="unnamed-chunk-8"><div class="rcode"><div class="source"><pre class="knitr r">c2_set <- getGmt("data/c2.cp.reactome.v4.0.symbols.gmt")
...
<div class="source"><pre class="knitr r">res$Direction <- ifelse(res$Direction=='Up', 1, -1)
...
In the <code class="knitr inline">34</code> modules with a FDR-q value 5% or less, a substantial number relate to cell cycle.
...
<div class="source"><pre class="knitr r">kable(resFDR[1:20,list(Direction, FDR, totalrank, set)]) 
</pre></div>
